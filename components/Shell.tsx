...
import {House,BriefcaseBusiness,UserRoundCog,ChartNoAxesCombined,BrainCircuit,Telescope,WandSparkles,RadioTower,SwatchBook,Send,FolderKanban,ScanSearch,ListTodo,CalendarRange,ChartSpline,Sparkles,SlidersHorizontal,Sun,Moon,LogOut,PanelLeftClose,PanelLeftOpen,Bell,CheckCheck,ExternalLink} from "lucide-react";
...
const icons=[House,BriefcaseBusiness,UserRoundCog,ChartNoAxesCombined,BrainCircuit,Telescope,WandSparkles,RadioTower,SwatchBook,Send,FolderKanban,ScanSearch,ListTodo,CalendarRange,ChartSpline,Sparkles,SlidersHorizontal];
const groups=[
 {label:"Visão e campanha",paths:["/dashboard","/campanhas","/candidatos","/comparativo"]},
...
export default function Shell({children,title}:{children:React.ReactNode,title:string}){
 ...
 function item(href:string,label:string,closeMobile=false){const i=menu.findIndex(x=>x[0]===href),Icon=icons[i];return <Link key={href} href={href} title={label} scroll={false} onClick={()=>closeMobile&&setMobile(false)} className={`nav-item ${p===href?'active':''}`}><span className="nav-icon"><Icon size={19} strokeWidth={1.8}/></span>{open||closeMobile?<span className="flex-1 truncate">{label}</span>:null}</Link>}
 function groupedNav(closeMobile=false){return groups.map(group=>{const items=visibleMenu.filter(([href])=>(group.paths as readonly string[]).includes(href));if(!items.length)return null;return <div className="nav-group" key={group.label}>{(open||closeMobile)&&<div className="sidebar-label">{group.label}</div>}{items.map(([href,label])=>item(href,label,closeMobile))}</div>})}
 return <div className="min-h-screen flex">
  <aside className={`${open?'sidebar-open':'sidebar-closed'} app-sidebar hidden md:flex`}><div className="sidebar-shell h-full flex flex-col"><div className="sidebar-brand"><div className="flex items-center gap-3 min-w-0"><div className="logo-mark shrink-0">A</div>{open&&<div className="min-w-0"><div className="font-black text-lg tracking-[.12em] truncate">ARCANUM</div><div className="text-[10px] muted">STRATEGY OS</div></div>}</div><button aria-label={open?"Recolher menu":"Expandir menu"} className="icon-btn shrink-0" onClick={()=>setOpen(!open)}>{open?<PanelLeftClose size={18}/>:<PanelLeftOpen size={18}/>}</button></div>
   <nav ref={navRef} className="sidebar-nav">{groupedNav()}</nav>
   <div className="sidebar-user"><div className="avatar">{access.admin?"JA":"MK"}</div>{open&&<div className="min-w-0"><div className="text-sm font-semibold truncate">{access.admin?"Administrador":"Marketing 2026"}</div><div className="text-xs muted truncate">{email||"Supabase conectado"}</div></div>}</div>
  </div></aside>
  {mobile&&<div className="fixed inset-0 z-50 bg-black/55 backdrop-blur-sm md:hidden" onClick={()=>setMobile(false)}><div className="mobile-sidebar" onClick={e=>e.stopPropagation()}><div className="flex items-center justify-between mb-5"><div className="flex items-center gap-3"><div className="logo-mark">A</div><b>ARCANUM</b></div><button className="icon-btn" onClick={()=>setMobile(false)}><PanelLeftClose size={18}/></button></div><nav className="sidebar-nav">{groupedNav(true)}</nav></div></div>}
  <main className={`${open?'md:ml-[292px]':'md:ml-[92px]'} flex-1 min-w-0 transition-all duration-300`}><header className="topbar"><div className="flex gap-3 items-center min-w-0"><button className="md:hidden icon-btn" aria-label="Abrir navegação" onClick={()=>setMobile(true)}><PanelLeftOpen size={19}/></button><div className="min-w-0"><h1 className="text-xl font-bold tracking-tight truncate">{title}</h1><p className="text-xs muted truncate">Inteligência estratégica <span className="mx-1">•</span> Ambiente operacional</p></div></div><div className="flex gap-2 items-center"><CampaignSelector/><CommandPalette/><div className="relative"><button className="icon-btn relative" aria-label="Notificações" onClick={()=>setNotificationOpen(v=>!v)}><Bell size={18}/>{unread>0&&<span className="notification-count">{unread>9?"9+":unread}</span>}</button>{notificationOpen&&<div className="notification-popover"><div className="p-4 border-b border-[var(--border)] flex items-center justify-between"><div><strong>Notificações</strong><p className="text-xs muted">Alertas do Radar Político</p></div><button className="text-xs flex items-center gap-1 muted" onClick={markAll}><CheckCheck size={15}/> Marcar como lidas</button></div><div className="max-h-[420px] overflow-auto">{notifications.map(item=><Link key={item.id} href={item.href||"/clipping"} onClick={()=>openNotification(item)} className={`notification-item ${item.read?"opacity-65":""}`}><span className={`notification-dot ${item.read?"read":""}`}/><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><strong className="text-sm">{item.title}</strong><ExternalLink size={14} className="shrink-0 muted"/></div><p className="text-xs mt-1 line-clamp-2">{item.message}</p><span className="text-[11px] muted mt-2 block">{when(item.createdAt)}</span></div></Link>)}{!notifications.length&&<div className="p-10 text-center muted"><Bell className="mx-auto mb-3"/><p className="text-sm">Nenhuma notificação.</p></div>}</div></div>}</div><button className="icon-btn" aria-label="Alternar tema" onClick={()=>setTheme(resolvedTheme==='dark'?'light':'dark')}>{resolvedTheme==='dark'?<Sun size={18}/>:<Moon size={18}/>}</button><button onClick={logout} className="icon-btn" aria-label="Sair"><LogOut size={18}/></button></div></header><div className="p-4 md:p-8 max-w-[1680px] mx-auto">{children}</div></main>
 </div>
}